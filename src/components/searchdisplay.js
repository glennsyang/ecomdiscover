...
const SearchDisplay = ({ props }) => {
    ...
    if (props.type === 'companies') {
        ...
        return (
            <Link key={id} to={slug} className="block px-4 py-2 text-gray-800 hover:bg-gray-100 hover:text-blue-500">
                <div className="flex flex-row antialiased">
                    <div className="w-1/5 border-r pr-2">
        ...
        return (
            <Link key={id} to={slug} className="block px-4 py-2 text-gray-800 hover:bg-gray-100 hover:text-blue-500">
                <div className="flex flex-row antialiased">
                    <div className="w-1/5 flex flex-col border-r pr-2">
                        <p className="text-gray-500 text-sm font-semibold text-right">
                            {compObj.name}
                        </p>
                    </div>
                    <div className="w-4/5 flex flex-col pl-2">
                        <div className="flex flex-row">
                            <p className="text-black text-base font-bold mr-2">{title}</p>
                            <Tag tags={tags} />
                        </div>
